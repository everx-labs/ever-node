...
pub struct RempNode {
    ...
    receipts_sender: tokio::sync::OnceCell<tokio::sync::mpsc::UnboundedSender<(Arc<KeyId>, RempReceipt, Vec<u8>)>>,
...
impl RempNode {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        start_receipts_worker(
            Arc::new(SendCombinedReceiptByAdnl::new(self.local_key.clone(), self.adnl.clone())),
            receiver,
...
impl Subscriber for RempNode {
    ...
    async fn try_consume_custom(&self, data: &[u8], peers: &AdnlPeers) -> Result<bool> {
        match deserialize_boxed(data) {
            Ok(object) => {
                if let Ok(combined_receipt) = object.downcast::<RempCombinedReceipt>() {
                    let subscriber = self.receipts_subscriber.get()
                        .ok_or_else(|| error!("receipts_subscriber is not set"))?;
...
impl SendCombinedReceiptByAdnl {
    pub fn new(local_key: Arc<KeyId>, adnl: Arc<AdnlNode>) -> Self {
        Self {
            adnl,
            local_key,
            #[cfg(feature = "telemetry")]
...
impl SendCombinedReceipt for SendCombinedReceiptByAdnl {
    async fn send_combined_receipt(
        ...
        let peers = AdnlPeers::with_keys(self.local_key.clone(), to.clone());
...
fn start_receipts_worker(
    ...
    mut receiver: tokio::sync::mpsc::UnboundedReceiver<(Arc<KeyId>, RempReceipt, Vec<u8>)>,
    receipts_in_channel: Arc<AtomicU64>,
    #[cfg(feature = "telemetry")]
...
async fn receipts_worker(
    sender: Arc<dyn SendCombinedReceipt>,
    receiver: &mut tokio::sync::mpsc::UnboundedReceiver<(Arc<KeyId>, RempReceipt, Vec<u8>)>,
    receipts_in_channel: Arc<AtomicU64>,
    #[cfg(feature = "telemetry")]
    telemetry: Arc<RempCoreTelemetry>,
) -> Result<()> {
    ...
    loop {
        match tokio::time::timeout(
            ...
            Ok(Some((to, receipt, sign))) => {
                receipts_in_channel.fetch_sub(1, Ordering::Relaxed);
                log::debug!("ReceiptsSender::worker: New iteration - receipt for {:x}", receipt.message_id());
                if add_receipt_to_map(&mut receipts, to, receipt, sign)? {
                    pending_receipts += 1;
                    #[cfg(feature = "telemetry")]
        ...
        if prev_packet_built_iter.elapsed().as_millis() >= BUILD_PACKET_PERIOD_MS {
            ...
            for (node_id, (node_updated_at, node_receipts)) in receipts.iter_mut() {
                ...
                loop {
                    ...
                    if let (Some(receipt), Some(data)) = (build_result.receipt.as_ref(), build_result.receipt_data.as_ref()) {
                        let timeout = node_updated_at.elapsed().as_millis() as u64 > RECEIPTS_SEND_PERIOD_MS;
                        if build_result.is_full_packet || timeout {
                            let now = Instant::now();
                            sender.send_combined_receipt(&node_id, &data).await?;
                            log::debug!(
