...
impl ControlQuerySubscriber {
    ...
    fn statistics_to_json(
        ...
        for item in map.iter() {
            let value = match *item.val() {
                0 => -1,
                value => now - value as i64
            };
            json_map.insert(item.key().to_string(), value.into());
        }
    ...
    async fn get_stats(&self) -> Result<Stats> {
        ...
        Self::add_stats(&mut stats, "node_status", sync_status);
        ...
        let value = Self::statistics_to_json(engine.last_validation_time(), now as i64);
        Self::add_stats(&mut stats, "last_validation_ago_sec", value);

        let value = Self::statistics_to_json(engine.last_collation_time(), now as i64);
