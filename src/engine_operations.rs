...
//        self.db().store_block_processed_in_ext_db(handle)?;
    ...
    async fn sign_and_send_remp_receipt(&self, to: Arc<KeyId>, receipt: RempReceipt) -> Result<()> {
        ...
        let key = self.network.get_validator_key(&validators).await?.ok_or_else(|| error!("Can't get validator's key"))?;
        ...
        self.network().remp().combine_and_send_receipt(to, receipt, signature).await
