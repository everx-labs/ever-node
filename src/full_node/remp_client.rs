...
impl RempClient {
    ...
    pub fn process_remp_message(
        ...
        tokio::spawn(async move {
            let spawned = started.elapsed().as_millis();
            // process message
            match self.clone().process_remp_message_impl(raw_message, id).await {
                Ok((processing_ns, sending_ns)) => {
                    log::info!(
                        "Processed REMP message {:x}, spawn: {:>6}ms, processing {:>6}ms, sending {:>6}ms, total {:>6}ms",
                        id, spawned, processing_ns / 1000000, sending_ns / 1000000, started.elapsed().as_millis()
                    );
                }
                Err(e) => {
    ...
    async fn process_remp_message_impl(
        ...
        match self.send_remp_message(validators, remp_message).await {
            Err(e) => {
                ...
                return Ok((processing_ns, sending_ns))
            },
            Ok(sent_to) => {
