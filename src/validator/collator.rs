...
impl ExecutionManager {
    ...
    fn start_account_job(
        ...
        let index_provider = self.index_provider.clone();
        ...
        let handle = tokio::spawn(async move {
            while let Some(new_msg) = receiver.recv().await {
                ...
                let params = ExecuteParams {
                    ...
                    debug,
                    index_provider: index_provider.clone(),
...
impl Collator {
    ...
    async fn do_collate(
        ...
        let index_provider = if let Some(shards) = collator_data.shards.clone() {
            let shards = ShardHashesStuff::from(shards);
            self.engine.acc_hashes_index_provider(shards, mc_data.state.block_id(), None)?
        } else { None };
            ...
            if let Some(addr) = &config_addr {
                if addr == &account_id {
                    let new_config_root = account
                        ...
                        .reference(0)?;
                    collator_data.new_config_opt = Some(ConfigParams::with_address_and_params(config_addr.clone().unwrap(), Some(new_config_root)));
                }
            }
    ...
    fn create_mc_state_extra(
        ...
        let (config, is_key_block) = if let Some(new_config) = collator_data.new_config_opt.take() {
