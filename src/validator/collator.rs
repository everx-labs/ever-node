...
use ever_block::{
    ...
    ConnectedNwDescrExt, ConnectedNwDescr, Account, GetRepresentationHash, HashmapRemover,
    EnqueuedMsg, SliceData, error, fail, AccountId, Cell, HashmapType, Result, UInt256, UsageTree
};
use ever_executor::{
...
struct CollatorData {
    ...
    /// * key - msg_sync_key
    /// * value - ext msg id
    accepted_ext_messages_buffer: HashMap<usize, (UInt256, i32)>,
    /// * key - msg_sync_key
...
impl CollatorData {
    ...
    fn add_accepted_ext_message_to_buffer(&mut self, src_msg_sync_key: usize, msg_id: UInt256, workchain_id: i32) {
        self.accepted_ext_messages_buffer.insert(src_msg_sync_key, (msg_id, workchain_id));
    }
    ...
    fn commit_accepted_ext_messages(&mut self) {
        for (_, (msg_id, workchain_id)) in self.accepted_ext_messages_buffer.drain() {
            self.accepted_ext_messages.push((msg_id, workchain_id));
        }
    }
    ...
    fn commit_new_messages(&mut self) {
        ...
        while let Some((_, msgs)) = self.new_messages_buffer.pop_first() {
            for new_msg in msgs {
                if let Ok(msg) = new_msg.msg.get_std() {
                    log::trace!(
                        ...
                        msg.int_header().map(|h| h.bounced),
                        msg.src().unwrap_or_default().address(),
                        msg.dst().unwrap_or_default().address(),
                    );
...
impl ExecutionManager {
    ...
    pub async fn execute(
        ...
        self.append_msgs_queue(msg_sync_key, &account_id, msg_hash_opt);
    ...
    fn start_account_job(
        ...
        let handle = tokio::spawn(async move {
            while let Some(new_msg) = receiver.recv().await {
                ...
                shard_acc.apply_transaction_res(
                    new_msg.0,
                    tx_last_lt,
                    &mut transaction_res,
                    account_root.clone()
    ...
    fn finalize_transaction(
        ...
        if let AsyncMessage::Ext(msg, msg_id) = new_msg {
            ...
            if let Err(err) = transaction_res {
                ...
                collator_data.add_rejected_ext_message_to_buffer(*msg_sync_key, (msg_id.clone(), err.to_string()));
...
impl Collator {
    ...
    async fn create_special_transaction(
        ...
        let msg_hash_opt = Some(msg.hash()?);
        let msg = CommonMessage::Std(msg);
        exec_manager.execute(
            account_id,
            f(msg),
            msg_hash_opt,
        ...
        while let Some(k_v) = iter.next() {
            ...
            if collator_data.out_msg_queue_info.already_processed(&enq)? {
                ...
                if to_us {
                    ...
                    let msg_sync_key = exec_manager.execute(
                        account_id.clone(),
                        msg,
                        Some(key.hash.clone()),
                        prev_data,
    ...
    async fn process_inbound_external_messages(
        ...
        while let Some((msg, msg_id)) = iter.next() {
            self.check_stop_flag()?;
            ...
            let msg = AsyncMessage::Ext(CommonMessage::Std(msg.deref().clone()), msg_id.clone());
            let msg_sync_key = exec_manager.execute(
                account_id.clone(),
                msg,
                Some(msg_id.clone()),
            ...
            log::debug!(
                ...
                self.collated_block_descr, msg_id, msg_sync_key, account_id,
            );
        }
        exec_manager.wait_transactions(collator_data).await?;
        ...
        while let Some((msg, id)) = remp_collator_interface.get_next_message_for_collation().await? {
            ...
            if self.shard.contains_address(&header.dst)? {
                ...
                if !collator_data.block_limit_status.fits_normal(REMP_CUTOFF_LIMIT, pruned_count) {
                    ...
                    let msg = AsyncMessage::Ext(msg_std, id.clone());
                    let msg_sync_key = exec_manager.execute(
                        account_id.clone(),
                        msg,
                        Some(id.clone()),
    ...
    async fn process_new_messages(
        ...
        while !stop_processing && !collator_data.new_messages.is_empty() {
            ...
            while let Some(NewMessage{ lt_hash: (created_lt, hash), msg, tr_cell, prefix }) = new_messages.pop() {
                ...
                let msg_hash_opt = Some(env.message_hash());
    ...
    async fn finalize_block(
        ...
        if workchain_id != -1
            && (collator_data.dequeue_count != 0 || collator_data.enqueue_count != 0
                ...
                || collator_data.transit_count != 0 || changed_accounts.len() != 0 || collator_data.remove_count != 0
            )
        {
            log::debug!(
                "{}: finalize_block finished: dequeue_count: {}, enqueue_count: {}, in_msg_count: {}, out_msg_count: {}, \
                execute_count: {}, transit_count: {},  changed_accounts: {}, remove_count: {} msg_queue_depth_sum: {}",
                ...
                collator_data.remove_count, collator_data.msg_queue_depth_sum
            );
        }
        log::trace!(
            "{}: finalize_block finished: dequeue_count: {}, enqueue_count: {}, in_msg_count: {}, out_msg_count: {}, \
            execute_count: {}, transit_count: {}, changed_accounts: {}, remove_count: {}, data len: {}",
            self.collated_block_descr, collator_data.dequeue_count, collator_data.enqueue_count,
            collator_data.in_msg_count, collator_data.out_msg_count, collator_data.execute_count,
            collator_data.transit_count, changed_accounts.len(), collator_data.remove_count, candidate.data.len()
        );
