 ...
 */
    ...
    fn precheck_one_account_block(base: &ValidateBase, acc_id: &UInt256, acc_blk: AccountBlock, engine: &Arc<dyn EngineOperations>) -> Result<()> {
        ...
        let mut there_were_internal_message = false;
            ...
            if base.config_params.has_capability(GlobalCapabilities::CapRemp)  {
                if let Some((id, is_internal)) = msg_info {
                    Self::check_message_ordering(
                        base, &id, is_internal, &mut there_were_internal_message, &mut prev_msg_hash, engine,
    ...
    fn check_message_ordering(
        ...
        id: &UInt256,
        is_internal: bool,
        there_were_internal_message: &mut bool,
        prev_hash: &mut Option<UInt256>,
        engine: &Arc<dyn EngineOperations>,
    ) -> Result<()> {
        if !is_internal && *there_were_internal_message {
            reject_query!("this transaction processed external message, but has LT greater than \
                at least one transaction with internal message.");
        } else if is_internal {
            *there_were_internal_message = true;
        }
        if !is_internal {
