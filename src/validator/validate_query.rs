...
struct ValidateBase {
    ...
    index_provider: Option<Arc<dyn ton_vm::executor::IndexProvider>>,
 ...
 */
    ...
    async fn init_mc_data(&mut self, base: &mut ValidateBase) -> Result<McData> {
        ...
        if base.shard().is_masterchain() {
            let shards = ShardHashesStuff::from(self.new_mc_shards.clone());
            let key = Some(self.block_candidate.block_id.root_hash());
            base.index_provider = 
                self.engine.acc_hashes_index_provider(shards, mc_data.state.block_id(), key)?
        }
 ...
 */
    ...
    fn check_one_transaction(
        ...
        let params = ExecuteParams {
            ...
            last_tr_lt: tr_lt,
            seq_no: base.info.seq_no(),
            seed_block: base.extra.rand_seed().clone(),
            index_provider: base.index_provider.clone(),
            debug: false,
