...
fn get_validator_descrs_by_collator_range(
    vset: &ValidatorSet,
    possible_validators: &Vec<CollatorRange>) -> Result<Option<ValidatorSubsetInfo>>
    cc_seqno: u32,
...
pub fn calc_subset_for_workchain_fast_finality(
    ...
    match try_calc_subset_for_workchain_fast_finality(
        vset, cc_seqno, mc_state, shard_id, block_seqno
    )? {
        Some(x) => Ok(x),
        None =>
            fail!(
