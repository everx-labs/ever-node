...
#[cfg(test)]
use crate::{db::memorydb::MemoryDb, tests::utils::create_storage_allocated};
...
impl ShardStateDb {
    ...
    #[cfg(test)]
    pub fn current_boc_db(&self) -> Result<&Arc<DynamicBocDb>> {
        let current = self.current_dynamic_boc_db_index.load(Ordering::Relaxed) as usize;
        match current {
